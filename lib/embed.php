 ...
 */
function flowplayer_embed_register_embed_handler() {
	wp_embed_register_handler(
		'flowplayer_embed',
		'#https?://play\.flowplayer\.com/api/video/embed\.jsp\?id=([a-zA-Z0-9_-]+)&(?:amp;)?pi=([a-zA-Z0-9_-]+)#i',
		'flowplayer_embed_embed_handler'
	);
}
add_action( 'init', 'flowplayer_embed_register_embed_handler' );

/**
 * Convert the recognised embed url to the actual embed code
 */
function flowplayer_embed_embed_handler( $matches, $attr, $url, $rawattr ) {
	$embed = sprintf(
	...
	return apply_filters( 'embed_flowplayer_embed', $embed, $matches, $attr, $url, $rawattr );
}
